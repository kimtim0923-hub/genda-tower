...
import {
  ...
  Users, Code2, BarChart3,
  Flag, TrendingUp, ChevronDown, ChevronRight,
} from "lucide-react";
...
const INIT_WS: Workstream[] = [
  { id:"landing", label:"🚀 랜딩페이지 & 유효성 검증", icon:<Globe size={12}/>, color:"landing", tasks:[
    {id:"l1",name:"랜딩페이지 와이어프레임 설계",   start:"2026-03-06",end:"2026-03-10",status:"active",  priority:"critical",note:"Aha Moment '즉시 견적 산출' 섹션 중심"},
    {id:"l2",name:"Next.js 랜딩페이지 개발",         start:"2026-03-10",end:"2026-03-22",status:"todo",   priority:"critical",note:"히어로 + 요금제 + CTA 폼"},
  ...
  { id:"linkedin", label:"🔗 LinkedIn Build in Public", icon:<Linkedin size={12}/>, color:"linkedin", tasks:[
    {id:"li1",name:"LinkedIn 프로필 최적화 (PCO→SaaS Founder)",start:"2026-03-06",end:"2026-03-10",status:"active",priority:"critical"},
...
// ─── 메인 ────────────────────────────────────────────────────────
export default function GendaControlTower() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f8fafc",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",fontSize:13}}>
      ...
      {/* 헤더 */}
      <header style={{background:"#fff",borderBottom:"1px solid #e2e8f0",position:"sticky",top:0,zIndex:40,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
        <div style={{maxWidth:1600,margin:"0 auto",padding:"10px 20px",display:"flex",alignItems:"center",gap:14}}>
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            ...
            <div>
              <div style={{fontWeight:900,fontSize:15,lineHeight:1.1}}>Gen-da 관제탑</div>
              <div style={{fontSize:9,color:"#94a3b8",marginTop:1}}>K-Popup SaaS · 1,000명 베타 모객</div>
            </div>
          </div>
          ...
          <div style={{textAlign:"right",flexShrink:0}}>
            ...
            <div style={{fontSize:10,fontWeight:700,color:"#4f46e5"}}>완료 {doneN} · 진행 {actN} / 전체 {allT.length}</div>
          </div>
        </div>
      </header>
      ...
      <main style={{maxWidth:1600,margin:"0 auto",padding:"14px 20px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:10,flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            ...
            <span style={{fontSize:9,color:"#94a3b8"}}>오늘 ({todayStr})</span>
          </div>
        ...
        {/* 타임라인 헤더 */}
        <div style={{display:"flex",alignItems:"center",background:"#fff",border:"1px solid #e2e8f0",borderRadius:9,padding:"5px 8px",marginBottom:5,position:"sticky",top:60,zIndex:30,boxShadow:"0 1px 2px rgba(0,0,0,.04)"}}>
          ...
          return (
            <div key={w.id} style={{border:`1px solid ${cfg.hdr}35`,borderRadius:9,overflow:"hidden",marginBottom:7,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
              ...
              {isOpen && w.tasks.map(t=>{
                ...
                const barBg=t.status==="done"?cfg.bar:t.status==="active"?cfg.bar:cfg.bar+"55";
        ...
        <div style={{marginTop:10,background:"#fff",borderRadius:10,padding:"12px 14px",border:"1px solid #e2e8f0"}}>
          ...
          <div style={{position:"relative",height:38}}>
            ...
            <div style={{position:"absolute",top:"40%",left:0,height:2,background:"linear-gradient(90deg,#4f46e5,#db2777,#d97706)",width:todayX+"%",transition:"width .4s"}}/>
            {MILESTONES.map(m=>(
        ...
        <div style={{textAlign:"center",fontSize:9,color:"#cbd5e1",marginTop:12,paddingBottom:16}}>
          Gen-da Control Tower · Sora (10Y PCO) · Agency-as-a-SaaS · 1,000 Beta Goal
        </div>
      </main>
    </div>
  );
